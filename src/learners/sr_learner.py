...
class SRLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            ...
            ans_adv = self.mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot, max_q_i=max_action_qvals, is_v=False)
        ...
        if self.role_mixer is not None:
            ...
            chosen_role_qvals = self.role_mixer(chosen_role_qvals, role_states, is_v=True)
            role_states = th.cat([role_states[:, 1:], role_states[:, 0:1]], dim=1)
            target_role_max_qvals = self.target_role_mixer(target_role_max_qvals, role_states, is_v=True)
